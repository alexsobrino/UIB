...
{
	/*int n_bloque_mb = n_bloque/(TAM_BLOQUE*8);
	int n_byte = (n_bloque/8) - (n_bloque_mb*TAM_BLOQUE);
	int n_bit = n_bloque%8;
	char buffer[TAM_BLOQUE];
	memset(buffer,0,TAM_BLOQUE);
	char mascara = 128>>n_bit;
	//esperar_sem(mutex,0);
	memcpy(buffer,&mb[n_bloque*TAM_BLOQUE],TAM_BLOQUE);
	//senalizar_sem(mutex,0);
	//char tmp = buffer[n_byte] & mascara;
	if (buffer[n_byte] & mascara > 0) {*/
	int n_bit = n_bloque % 8;
...
int escribir_mb (int n_bloque, int valor)
{
	...
	char mascara = 128>>n_bit;
	//esperar_sem(mutex,0);
	memcpy(buffer,&mb[n_bloque_mb*TAM_BLOQUE],TAM_BLOQUE);
	if (valor == 0) {*/
	int n_bit = n_bloque % 8;
	if (valor == 0) {
		if (DEBUG == 1) {
		...
		*(mb+(n_bloque/8)) = *(mb+(n_bloque/8)) & (~(128>>n_bit));
		//buffer[n_byte] = buffer[n_byte] & (~mascara);
	} else if (valor == 1) {
		if (DEBUG == 1) {
	...
	//memcpy(&mb[n_bloque_mb*TAM_BLOQUE],buffer,TAM_BLOQUE);
	//if (escribir_bloque(sb->n_bloque_mb+n_bloque_mb,buffer) == -1) {
	//	printf("(ficheros_basico.c|escribir_mb) ERROR: imposible escribir bloque %d.\n",sb->n_bloque_mb+n_bloque_mb);
	//	return -1;
	//}
	//senalizar_sem(mutex,0);
	if (escribir_mb_sf() == -1) {
		printf("MEEEEEEEEEEEEEEC\n");
		return -1;
	}
	return 0;
}
